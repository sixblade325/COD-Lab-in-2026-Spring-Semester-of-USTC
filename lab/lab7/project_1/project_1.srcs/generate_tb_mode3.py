...
# 定义一些仿真文件参数
MODE = 3 # 0:随机读写 1：模拟CPU伪顺序读写（会有不定期跳转，概率为BranchP） 3：高局部性相关访问
BranchP = 0.1 # 跳转概率，仅在MODE=1时有效，当BranchP = 0时，为顺序读写不会跳转
HOT_REGION_NUM = 32 # MODE=3：热点区域数量
HOT_REGION_SIZE = 32 # MODE=3：每个热点区域包含的字地址数量
HOT_SWITCH_P = 0.03 # MODE=3：切换到其他热点区域的概率
HOT_JUMP_P = 0.20 # MODE=3：在当前热点区域内短距离跳转的概率
READ_NUM   = 2000 # 读取次数
WRITE_NUM  = 1000 # 写入次数
...
if MODE == 0:
    ...
    hot_region_size = min(HOT_REGION_SIZE, mem_size)
    hot_region_num = min(HOT_REGION_NUM, mem_size)
    hot_bases = []

    # 生成多个热点区域，访问地址多数集中在这些区域内。
    for i in range(hot_region_num):
        hot_bases.append(randint(0, mem_size-1))
    ...
    for i in range(READ_NUM+WRITE_NUM):
        if randint(0, 10000) < HOT_SWITCH_P*10000:
            current_hot = randint(0, hot_region_num-1)
            current_offset = randint(0, hot_region_size-1)
        elif randint(0, 10000) < HOT_JUMP_P*10000:
            # 热点区域内短跳转，模拟反复访问相近数据。
            current_offset = (current_offset + randint(-4, 4)) % hot_region_size
        else:
            # 默认顺序向前，保留空间局部性。
            current_offset = (current_offset + 1) % hot_region_size

        test_addr.append((hot_bases[current_hot] + current_offset) % mem_size)
