...
module Nwaycache_FIFO #(
    ...
    wire [WAY_NUM * LINE_WIDTH-1:0]  r_line;   // Data Bram读数据
    wire [LINE_WIDTH-1:0]  HIT_r_line;
    wire [LINE_WIDTH-1:0]  WB_r_line;
    ...
    wire [WAY_NUM * TAG_WIDTH-1:0]   r_tag;    // Tag Bram读数据 用于比较
    wire [TAG_WIDTH-1:0]   WB_r_tag;
    ...
    wire hit;    // Cache命中
    wire [WAY_WIDTH-1:0] hit_way;// Cache命中的路
    wire [WAY_NUM-1:0] hit_way_one_hot;//命中路onehot
    
    reg  [WAY_WIDTH - 1 : 0] way_using;//正在访问的路，用于WB_way_select
    wire [WAY_WIDTH - 1 : 0] WB_way_select;
    wire [WAY_NUM - 1 : 0] WB_way_select_one_hot;//WB选择写回的路onehot

    wire [0:0] clean;//CleanWayDet检测是否有!dirty路
    wire [WAY_WIDTH-1:0] clean_way;
    wire [WAY_NUM-1:0] clean_way_one_hot;
    reg  [0:0] clean_buf;
    reg  [WAY_WIDTH-1:0] clean_way_buf;
    reg  [WAY_NUM-1:0] clean_way_one_hot_buf;
    reg  [0:0] clean_buf_we;
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            if (clean_buf_we) begin
                clean_buf <= clean;
                clean_way_buf <= clean_way;
                clean_way_one_hot_buf <= clean_way_one_hot;
            end
    ...
    // 脏块地址计算
    assign dirty_mem_addr = {WB_r_tag, w_index}<<(LINE_OFFSET_WIDTH+SPACE_OFFSET);
    ...
    generate
        for (i=0; i<WAY_NUM; i=i+1)begin
            bram #(
                ...
                .we(tag_we[i]),
                .dout({valid[i], dirty[i], r_tag[i*TAG_WIDTH +:TAG_WIDTH]})
            );
            ...
            bram #(
                ...
                .din(w_line),
                .we(data_we[i]),
                .dout(r_line[i*LINE_WIDTH +:LINE_WIDTH])
            );
        end
    endgenerate
    ...
    LineMux #(
        ...
        .src(r_line),
        .way_one_hot(WB_way_select_one_hot),
        .res(WB_r_line)
    );//选择WB情形下的r_line
    ...
    HIT_DETECT #(
        ...
        .way_valid(valid),
        .hit(hit),
        .hit_way_one_hot(hit_way_one_hot),
        .hit_way(hit_way)
    );
    ...
    CleanWayDec #(
        ...
        .clean(clean),
        .clean_way(clean_way),
        .clean_way_one_hot(clean_way_one_hot)
    );
    ...
    WB_FIFO #(
        ...
        .way_using(way_using),
        .index(w_index),
        .way_select(WB_way_select),
        .way_select_one_hot(WB_way_select_one_hot)
    );
    ...
    assign w_line = (CS == IDLE && op_buf) ? ret_buf & ~w_line_mask | w_data_line : // 写入未命中，需要将内存数据与写入数据合并
                    ...
                    HIT_r_line & ~w_line_mask | w_data_line; // 写入命中,需要对读取的数据与写入的数据进行合并

    // 选择输出数据 从Cache或者从内存 这里的选择与行大小有关，因此如果你调整了行偏移位宽，这里也需要调整
    always @(*) begin
        case (word_offset)
            0: begin
                cache_data = HIT_r_line[31:0];
                mem_data = ret_buf[31:0];
            end
            1: begin
                cache_data = HIT_r_line[63:32];
                mem_data = ret_buf[63:32];
            end
            2: begin
                cache_data = HIT_r_line[95:64];
                mem_data = ret_buf[95:64];
            end
            3: begin
                cache_data = HIT_r_line[127:96];
        ...
        case(CS)
            IDLE: begin
                ...
                if(refill) begin
                    ...
                    w_dirty = 1'b0;
                    WB_we = 1'b1;
                    if(clean_buf)begin//这里写挂了：clean是根据当前r_index算出来的，miss时没有保存这个值，为0，而0可能是clean的
                        way_using = clean_way_buf;
                        data_we = clean_way_one_hot_buf;
                        tag_we = clean_way_one_hot_buf;
                    end else begin
                        way_using = WB_way_select;
                        data_we = WB_way_select_one_hot;
                        tag_we = WB_way_select_one_hot;
                    end
            ...
            READ: begin
                ...
                if (hit) begin // 命中
                    miss = 1'b0;
                    addr_buf_we = 1'b1; // 请求地址缓存写使能，为处理下一条指令做准备
                end else begin // 未命中
                    miss = 1'b1;
                    addr_buf_we = 1'b0; 
                    clean_buf_we = 1'b1;
                    if (!clean) begin // 脏数据需要写回
                        mem_w = 1'b1;
                        mem_addr = dirty_mem_addr;
                        mem_w_data = WB_r_line; // 写回数据
                    end
                end
            end
            MISS: begin
            ...
            WRITE: begin
                ...
                if (hit) begin // 命中
                    ...
                    w_valid = 1'b1;
                    w_dirty = 1'b1;
                    data_we = hit_way_one_hot;
                    tag_we = hit_way_one_hot;
                end else begin // 未命中
                    miss = 1'b1;
                    addr_buf_we = 1'b0; 
                    clean_buf_we = 1'b1;
            ...
            // ***********************************************
            // * Derek's note: 这里读未命中和写未命中的 dirty 情形与 W_DIRTY 状态虽有重复，
            // * 但这样写会快一个时钟周期。                   *
            // ***********************************************
            W_DIRTY: begin
                miss = 1'b1;
